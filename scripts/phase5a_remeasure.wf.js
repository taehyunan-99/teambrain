export const meta = {
  name: 'phase5a-remeasure-recall-rank',
  description: '표적 4질문(C1/C3/C4/A4)을 near-miss 증식 후 재측정 — recall(정답 후보 도달)과 rank(후보 중 정답 식별) 분리 채점',
  phases: [
...
// 재측정 대상: Phase 4 표적 4질문(near-miss로 grep 후보가 폭증한 것들).
// truth_files = 정답이 실제로 든 파일. 이걸 열었는지(recall)와 정답을 골랐는지(rank)를 분리.
const questions = [
  {
    id: 'C4', type: '건초더미-매몰',
    q: '추석 연휴 때 배포나 배치 일정 어떻게 챙겼어?',
    truth_files: ['raw/2025-09-02-weekly.md', 'raw/2025-09-09-weekly.md'],
    truth_fact: '도현이 인프라/정산에 핑 돌려 명절 전 점검. weekly 노이즈에 분산',
  },
  {
    id: 'C1', type: '건초더미-매몰',
    q: '로그 서버 디스크 꽉 찼던 적 있었나? 몇 퍼센트까지 갔었어?',
    truth_files: ['raw/infra/2024-10-24-weekly.md', 'raw/infra/slack/2024-10-23-infra-disk-cleanup.md'],
    truth_fact: '82%까지 참, 정리 후 54%. 2024-10 인프라 노이즈에만 존재',
  },
  {
    id: 'C3', type: '건초더미-매몰',
    q: '2025년 1월에 CI 빌드 간헐적으로 깨지던 문제 원인이 뭐였어?',
    truth_files: ['raw/infra/slack/2025-01-22-infra-ci-flaky-build-chatter.md'],
    truth_fact: '캐시 만료 시간 검증 테스트의 sleep 타이밍 의존. CI 러너 부하 때 flaky',
  },
  {
    id: 'A4', type: '고유명사-정밀',
