...
### 🔗 [teambrain.netlify.app](https://teambrain.netlify.app/)
...
> 가상 PG사 **Nimbus Pay**(결제·정산·인프라 3팀, 24개월) 슬랙로그를 시뮬레이션한 프로젝트.
...
- **near-miss** = 같은 제품명·약어·날짜를 공유하지만 맥락이 다른 닮은 오답.<br>예: 진짜 *2024-10-30 장애* vs *CI 풀 고갈* vs *다른 가맹점 온보딩 논의*.
...
| 대상 | 못 보는 것 |
|------|-----------|
| 세컨드브레인 / PKM | 입력이 깨끗하다는 전제라 **near-miss 문제 자체가 없다** |
| 일반 위키봇 / RAG | "retrieval만 고치면 된다"는 프레임이라 **reasoning이 사각지대** |

> 정직하게: "할루시네이션 방지"(RAG 기본기)나 "보안용 로컬 LLM"(제약 조건)은 약한 차별점이라 내세우지 않는다. 진짜 차별점은 **near-miss를 검색이 아닌 판단으로 가른다**는 한 가지다.
...
> **현 단계에선 구현하지 않고 설계로만 남겨둔다.** 자동화는 *증폭기*라, 판별 품질을 검증하기 전에 일일 자동 push를 켜면 잘못된 합성을 매일 팀 전체에 자동 방송하기 때문이다. 검증이 먼저, 자동화는 그 위에.
